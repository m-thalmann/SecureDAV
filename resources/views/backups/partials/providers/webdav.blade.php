...
<x-form-field name="targetUrl" class="md:w-2/3">
    ...
    <x-input name="targetUrl" type="url" :value="isset($configuration) ? $configuration->config['targetUrl'] : old('targetUrl')" placeholder="https://dav.example.com/directory1" required />
</x-form-field>

<x-form-field name="username" class="md:w-2/3">
    <x-slot name="label">{{ __('Username') }}</x-slot>

    <x-input name="username" :value="isset($configuration) ? $configuration->config['username'] : old('username')" placeholder="user1" required />
</x-form-field>

<x-form-field name="password" class="md:w-2/3">
    <x-slot name="label">{{ __('Password') }}</x-slot>

    <x-input name="password" type="password" placeholder="123" required />
</x-form-field>